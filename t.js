...
async function playMusic(track, pause = false) {
    ...
    if (!pause) {
        const mp=".mp3"
        currentSong.src = audioSource+mp;
...
async function getsongs(folder) {
    ...
    songUL.innerHTML = ""
    const folderName = `/songs/${folder}`.split("/")[3];
    const foldername=folderName.replaceAll("%20"," ")
    console.log(foldername)
    for (const song of songs) {
        songUL.innerHTML += `<li> 
        <i id="m" class="fa-solid fa-music"></i>
        <div class="songsinfo">
            <p1>Song Name : ${song.replaceAll("%20", " ").replaceAll(".mp3"," ")}</p1>
            <br>
            <p2>Artist Name : ${foldername}</p2>
        </div>
